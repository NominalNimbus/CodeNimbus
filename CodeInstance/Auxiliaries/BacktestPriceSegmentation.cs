...
namespace Auxiliaries
{
    public static class BacktestPriceSegmentation
    {
        public static List<TradeSignal> BacktestPriceSegmentProcessor(SignalBase signalBase, Dictionary<Selection, IEnumerable<Bar>> marketData,
                                                    ...
                                                    PriceConstants btBarSegment,
                                                    Dictionary<Selection, IEnumerable<Bar>> trigInstrData = null,
                                                    IEnumerable<Tick> queuedTicks = null)
        {
            var modMarketData = marketData.ToDictionary(k => (Selection)k.Key.Clone(), v => v.Value.Select(b => new Bar(b)).ToList().AsEnumerable());
            ...
            switch (btBarSegment)
            {
                case PriceConstants.OPEN:
                    ...
                    trades = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    break;
                ...
                case PriceConstants.OHLC:
                    ...
                    var trades1 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    trades = trades1;
                    ...
                    //signalBase.Alert($"mod closeBid: {modLast.CloseBid}");
                    //signalBase.Alert($"mod mean: {modLast.MeanClose}");
                    var trades2 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    ...
                    //signalBase.Alert($"mod closeBid: {modLast.CloseBid}");
                    //signalBase.Alert($"mod mean: {modLast.MeanClose}");
                    var trades3 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    ...
                    signalBase.Alert($"mod closeBid: {modLast.CloseBid}");
                    signalBase.Alert($"mod mean: {modLast.MeanClose}");
                    var trades4 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                ...
                case PriceConstants.OLHC:
                    ...
                    modLast.CloseBid = origLast.OpenBid;
                    trigInstrData = modMarketData;
                    trades1 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    trades = trades1;
                    ...
                    modLast.CloseBid = origLast.LowBid;
                    trigInstrData = modMarketData;
                    trades3 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    trades.AddRange(trades3);

                    modLast.CloseAsk = origLast.HighAsk;
                    modLast.CloseBid = origLast.HighBid;
                    trigInstrData = modMarketData;
                    trades2 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    trades.AddRange(trades2);

                    trigInstrData = modMarketData;
                    trades4 = TradeSignalDetection.DetectSignals(signalBase, modMarketData, tradeParams, trigInstrData, queuedTicks);
                    trades.AddRange(trades4);
                    break;
            }
